...
class MHCN(SocialRecommender,GraphRecommender):
    ...
    def buildMotifInducedAdjacencyMatrix(self):
        ...
        C9 = (Y.dot(Y.T)).multiply(U)
        A9 = C9 + C9.T
        A10  = Y.dot(Y.T)-A8-A9
        

        #四节点的联合模体+1
        A11  = (Y.dot(Y.T)) + (U.T.dot(U)) + (U.dot(Y).dot(Y.T)).multiply(U) + (Y.dot(Y.T).dot(U.T)).multiply(U.T)
        A12  = (Y.dot(Y.T)) + (B.dot(B)) + (Y.dot(Y.T).dot(B)).multiply(B) + (B.dot(Y).dot(Y.T)).multiply(B)
        A13  = (Y.dot(Y.T)) + (U.dot(U.T)) + (Y.dot(Y.T).dot(U)).multiply(U) + (U.T.dot(Y).dot(Y.T)).multiply(U.T)
        A14  = (Y.dot(Y.T)) + (U.T.dot(B)) + (B.dot(U)) + (Y.dot(Y.T).dot(B)).multiply(U.T) + (B.dot(Y).dot(Y.T)).multiply(U) + (Y.dot(Y.T).dot(U.T)).multiply(B) + (U.dot(Y).dot(Y.T)).multiply(B)
        A15  = (Y.dot(Y.T)) + (U.dot(B)) + (B.dot(U.T)) + (Y.dot(Y.T).dot(U)).multiply(B) + (U.T.dot(Y).dot(Y.T)).multiply(B) + (Y.dot(Y.T).dot(B)).multiply(U) + (B.dot(Y).dot(Y.T)).multiply(U.T)
        A16  = (Y.dot(Y.T)) + (U.dot(U)) + (U.T.dot(U.T)) + (Y.dot(Y.T).dot(U)).multiply(U.T) + (U.T.dot(Y).dot(Y.T)).multiply(U) + (Y.dot(Y.T).dot(U.T)).multiply(U) + (U.dot(Y).dot(Y.T)).multiply(U.T)
 
        #四节点的联合模体+1+2
        #A16  = (Y.dot(Y.T)).multiply(B) + (Y.dot(Y.T).dot(U.T)).multiply(B.dot(U.T)) + (U.dot(Y).dot(Y.T)).multiply(U.dot(B))
        #A17  = (Y.dot(Y.T)).multiply(B) + (Y.dot(Y.T).dot(U)).multiply(B.dot(U))
        #A18  = (Y.dot(Y.T)).multiply(U) + (Y.dot(Y.T)).multiply(U.T) + (Y.dot(Y.T).dot(U)).multiply(U.T.dot(U)) + (U.T.dot(Y).dot(Y.T)).multiply(U.T.dot(U))
        #A19  = (Y.dot(Y.T)).multiply(U) + (Y.dot(Y.T)).multiply(U.T) + (Y.dot(Y.T).dot(U.T)).multiply(U.T.dot(U.T)) + (U.dot(Y).dot(Y.T)).multiply(U.dot(U))
        #A20  = (Y.dot(Y.T)).multiply(U.T) + (Y.dot(Y.T).dot(U)).multiply(U.dot(U)) + (U.T.dot(Y).dot(Y.T)).multiply(U.T.dot(U.T))
        #A21  = (Y.dot(Y.T)).multiply(U.T) + (Y.dot(Y.T)).multiply(U) + (Y.dot(Y.T).dot(U.T)).multiply(U.dot(U.T)) + (U.dot(Y).dot(Y.T)).multiply(U.dot(U.T))
        #A22  = (Y.dot(Y.T)).multiply(B) + (B.dot(Y).dot(Y.T)).multiply(B.dot(B))
        #A23  = (Y.dot(Y.T)).multiply(U.T) + (Y.dot(Y.T)).multiply(U) + (Y.dot(Y.T).dot(B)).multiply(U.dot(B)) + (B.dot(Y).dot(Y.T)).multiply(B.dot(U.T))
        #A24  = (Y.dot(Y.T)).multiply(U) + (Y.dot(Y.T)).multiply(U.T) + (Y.dot(Y.T).dot(B)).multiply(U.T.dot(B)) + (B.dot(Y).dot(Y.T)).multiply(B.dot(U))

        #四节点的社交模体+1
        #A25  =  (U.T.dot(U.T).dot(U.T)).multiply(U) + (U.dot(U)).multiply((U.T).dot(U.T)) + (U.dot(U).dot(U)).multiply(U.T)
        #A26  =  (B.dot(B).dot(B)).multiply(B) + (B.dot(B)).multiply((B).dot(B)) 
        #A27  =  (U.T.dot(B).dot(U.T)).multiply(B) + (B.dot(U).dot(B)).multiply(U.T) + (B.dot(U)).multiply(U.T.dot(B)) + (U.dot(B)).multiply(B.dot(U.T))
        #A28  =  (B.dot(B).dot(U)).multiply(B) + (B.dot(U.T).dot(B)).multiply(B) + (U.T.dot(B).dot(B)).multiply(B) + (U.dot(B).dot(B)).multiply(B) + (B.dot(U).dot(B)).multiply(B) + (B.dot(B).dot(B)).multiply(U.T) + (B.dot(B).dot(B)).multiply(U) + (B.dot(B).dot(U.T)).multiply(B) + (B.dot(B)).multiply(B.dot(U.T)) + (B.dot(B)).multiply(U.dot(B)) + (B.dot(B)).multiply(U.T.dot(B)) + (B.dot(B)).multiply(B.dot(U))
        #A29  =  (U.T.dot(U).dot(U)).multiply(B) + (B.dot(U.T).dot(U.T)).multiply(U.T) + (B.dot(U.T)).multiply(U.T.dot(U)) + (U.T.dot(U.T).dot(U)).multiply(B) + (U.dot(U).dot(B)).multiply(U) + (U.dot(B)).multiply(U.T.dot(U)) + (B.dot(U.T)).multiply(U.T.dot(U.T)) + (B.dot(U.T).dot(U)).multiply(U.T) + (U.dot(B).dot(U.T)).multiply(U) + (U.dot(B)).multiply(U.dot(U)) + (U.T.dot(U).dot(B)).multiply(U) + (U.dot(B).dot(U.T)).multiply(U.T)

        #五节点的社交模体+1
        #A30 = (Y.dot(Y.T)).multiply(B.dot(B).dot(B)) + (B.dot(B)).multiply(Y.dot(Y.T).dot(B)) + (B).multiply(Y.dot(Y.T).dot(B).dot(B)) + (B).multiply(B.dot(Y).dot(Y.T).dot(B))
        #A31 = (Y.dot(Y.T)).multiply(U.T.dot(B).dot(U)) + (U.T).multiply(Y.dot(Y.T).dot(U.T).dot(B)) + (Y.dot(Y.T).dot(U.T)).multiply(U.T.dot(B))
        #A32 = (Y.dot(Y.T)).multiply(B.dot(U).dot(B)) + (B).multiply(Y.dot(Y.T).dot(B).dot(U.T)) + (Y.dot(Y.T)).multiply(B.dot(U.T).dot(B)) + (B).multiply(Y.dot(Y.T).dot(B).dot(U)) + (B.dot(U)).multiply(Y.dot(Y.T).dot(B)) + (B.dot(U.T)).multiply(Y.dot(Y.T).dot(B)) + (U).multiply(B.dot(Y).dot(Y.T).dot(B)) + (U.T).multiply(B.dot(Y).dot(Y.T).dot(B))
        #A33 = (Y.dot(Y.T)).multiply(U.T.dot(U).dot(U)) + (U.T).multiply(Y.dot(Y.T).dot(U.T).dot(U.T)) + (U.T.dot(U)).multiply(Y.dot(Y.T).dot(U.T)) + (U.T.dot(U.T)).multiply(Y.dot(Y.T).dot(U.T)) + (U.T).multiply(Y.dot(Y.T).dot(U.T).dot(U)) + (U).multiply(U.dot(Y).dot(Y.T).dot(U.T))


        #addition and row-normalization
        H_s = sum([A1,A2,A3,A4,A5,A6,A7])
        H_s = H_s.multiply(1.0/H_s.sum(axis=1).reshape(-1, 1))
        H_j = sum([A8,A9,A11,A12,A13,A14,A15,A16])
